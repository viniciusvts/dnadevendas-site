                        ...
                        <div class="row">
                            <ul class="col-md-12">
                                <li v-for="(post, index) in posts" :key="index">
                                    <router-link :to="{ name: 'PodcastSingle', params: { slug: post.slug, post: post } }" v-html="post.title.rendered">
                                    </router-link>
                                </li>
                            </ul>
                        </div>
...
export default {
    ...
    methods:{        
        ...
        showSearch(){
            let searchBtn  = document.querySelector('#podSearch');           
            searchBtn.onclick = () =>{
                this.show = true;
                setTimeout( () => {
                    this.exitSearch();
